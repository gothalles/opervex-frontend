...
const filterNavigation = (navigation, roles, user) => {
  if (!roles) return [];
  return navigation.reduce((filtered, item) => {
    if (item.command && !roles.some((x) => x.command === item.command))
      return filtered;

    if (item.admin) if (!user.administrator) return filtered;

    if (item.children) {
      const filteredChildren = filterNavigation(item.children, roles, user);
          ...
          {itemSubMenu.children.map((item, index) =>
            item.children ? (
              <SubMenu
                key={index}
                itemSubMenu={item}
                segmentTop={"/" + segmentTop + "/" + item.segment}
              />
            ) : (
              <NavItem key={index}>
                <NavLink
                  href={segmentTop + "/" + item.segment}
...
const NavbarMenu = () => {
  ...
  useEffect(() => {
    setFilteredNavigation(filterNavigation(NAVIGATION, roles, user));
  }, [roles]);
    ...
    <>
      ...
      >
        ...
        <Offcanvas.Body className="p-0">
          <Nav className="flex-column menu-body">
